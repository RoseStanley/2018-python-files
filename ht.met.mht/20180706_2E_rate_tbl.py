...
for k in range (len(mes)):
    ...
    for j in range (len(angles)):    
        angle = angles[j]   
        inpath = '~/Documents/file_simulations/tbl_20180719_01_trg/heppy/set_all/sel_030/020_counts/tbl_n.process.htbin.metbin.mhtbin.{}.txt'.format(angle)
        ...
        tbl1 = tbl.groupby(['process', 'htbin','mhtbin',angle])['n'].sum().reset_index()
...
#            df = dbg
#            htval = htvals[i]
#            df = df[df.mhtbin == htval]
#            df = df[df.process == 'QCD']
#            df2 = df.pivot_table('cumn', [angle], me)
#            df2.to_csv(r'20180719_01_rate.mhtbin{}.{}.{}_0{}0.csv'.format(htval, me, angle, sel), sep='\t', mode='w') #, float_format='%6f')
