...
htvals = [50, 100, 200, 400, 600, 800, 1000]

for k in range (len(mes)):
    me = mes[k]
    for j in range (len(angles)):    
        angle = angles[j]   
        inpath = '~/Documents/file_simulations/tbl_2018{}{}_01_trg/heppy/set_{}/sel_030/020_counts/tbl_n.process.htbin.{}.{}.txt'.format(month, date, folder, me, angle)
        ...
        dbg['cumn'] = dbg['n']
        dbg['cumn'] = dbg.groupby(['process', 'htbin', me])['cumn'].cumsum()
        dbg['cumn'] = dbg.groupby(['process', 'htbin', angle])['cumn'].cumsum()
        dbg['cumn'] = dbg.groupby(['process', me, angle])['cumn'].cumsum()
#        dbg['cumn'] = dbg.groupby(['htbin', me, angle])['cumn'].cumsum()
...
#            df2.to_csv(r'{}{}_rate.htbin{}.{}.{}.csv'.format(month, date, htval, me, angle), sep='\t', mode='w') #, float_format='%6f')
#        
